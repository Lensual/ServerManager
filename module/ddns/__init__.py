        ...
        for user in Cloudflare_list.keys():
            ...
            for domain in user["domains"].keys():
                ...
                if domain["enable"]==True:
                    cf.update_DNS_Record(domain["prefix"],domain["domain"],new_ip,domain["type"])
...
class IPv6_Toolkit():
    ...
    def get_IPv6_Addr_by_DefaultRoute():#通过默认路由获取/128地址
        getip=os.popen("route -A inet6 -n|grep /128|grep -v fe80::|grep -v ::1/128")
...
class Cloudflare():
    ...
    def update_DNS_Record(self,prefix,domain,new_content,type):
        ...
        if result["success"]:
            logger.info("Successfully Update DNS record of "+name+", from "+record["content"]+" to "+new_content)
        else:
            logger.error("Update DNS record of "+name+" failed! Error+"+result["errors"]["code"]+":"+result["errors"]["message"])
    ...
    def get_RecordID(self):
        for domain in self.zone_id.keys():
            ...
            for record in record_list:
                self.record_id[domain].append({"name":record["name"],"id":record["id"],"type":record["type"],"content":record["content"]})
